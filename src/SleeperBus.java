...
public class SleeperBus extends Bus
{
	...
	public void showStatus(String email)
	{
		...
		try
		{
			String sql="select * from sleeper_seatings where bus_id=?  order by row_id";
			...
			while(rs.next())
			{
				...
				System.out.println(rs.getString("row_id")+"	"+rs.getString("A")+"	"+rs.getString("B")+"	");
		...
		for(int i=1;i<=n;i++)
		{
			...
			do 
			{
				...
				else
				{
					...
					try
					{	
						String sql="select "+column+" from sleeper_seatings where bus_id=? AND row_id=?";
						PreparedStatement st=Main.dbConnection.prepareStatement(sql);
						st.setString(1,getId());
						st.setInt(2,row);
						ResultSet rs=st.executeQuery();
						...
						if(t!=0)
						{
							sql="select "+column+" from sleeper_seatings where bus_id=? AND row_id=?";
		...
		for(int i=0;i<n;i++)
		{
			try
			{
				...
				String sql="update sleeper_seatings set "+column+"=? where bus_id=? AND row_id=?";
				PreparedStatement st=Main.dbConnection.prepareStatement(sql);
	...
	public void cancelSeat(String row,String column)
	{
		try 
		{
			String sql="update sleeper_seatings set "+column+"='A' where bus_id=? AND row_id=?";
			PreparedStatement st=Main.dbConnection.prepareStatement(sql);
			st.setString(1,getId());
